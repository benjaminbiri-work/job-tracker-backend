...
router.get("/dashboard", async (req, res) => {
  const cards = await pool.query(`
    select
      ...
      (select count(*)::int from interviews i join profiles p on p.id = i.profile_id where p.assigned_user_id = $1 and i.due_date >= current_date and i.due_date < current_date + interval '7 days') as this_week_interviews
  `, [req.user.id]);
  ...
  const rows = await pool.query(`
    select
      ...
      count(case when i.due_date >= current_date and i.due_date < current_date + interval '7 days' then 1 end)::int as this_week_interviews
    from profiles p
...
router.get("/profiles", async (req, res) => {
  const result = await pool.query("select * from profiles where assigned_user_id = $1 order by name asc", [req.user.id]);
...
router.get("/profiles/:profileId/apply-data", async (req, res) => {
  ...
  const allowed = await pool.query("select * from profiles where id = $1 and assigned_user_id = $2", [profileId, req.user.id]);
  const profile = allowed.rows[0];
  if (!profile) return res.status(403).json({ message: "Profile not assigned to you." });
